...
private:
	...
	mutex locker;
	condition_variable pusher;
	condition_variable popper;
...
public:
	...
	void push(char* data, int len){
		...
		pusher.wait(locked, [this]{return q.size() < cap;});
		//2. Convert the incoming byte sequence given by data and len into a vector<char>
		vector<char> adder(data, data + len);
		//3. Then push the vector at the end of the queue
		q.push(adder);
		locked.unlock();
		popper.notify_one();
		//cout << "Pushed... Size: " << q.size() << endl;
	}

	int pop(char* buf, int bufcap){
		//1. Wait until the queue has at least 1 item
		unique_lock<mutex> locked (locker);
		popper.wait(locked, [this]{return q.size() > 0;});
